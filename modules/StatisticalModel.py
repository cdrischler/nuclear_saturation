...
class StatisticalModel:
    ...
    def plot(self, ax=None, num_samples=5000, kind="predictive_y", based_on="both", plot_data=True, levels=None,
             set_xy_limits=True, set_xy_lbls=True, place_legend=True, validate=False):
        """
        plots the confidence region of the samples generated by `sample()`.
        For the prior predictive and posterior predictive, which are bivariate t-distributions, we have
        derived the confidence regions (i.e., ellipses) analytically.
        For the other distribution functions, we instead use Kernel Density Estimation.
        (see also `sample()` for more details on the arguments)
        #TODO: The confidence regions for `marginal_m` could also be plotted analytically.
        ...
        if based_on == "both":
            task_list = tmp
        elif based_on in tmp:
            task_list = (based_on, )
        else:
            raise ValueError(f"'{based_on} is unsupported as 'based_on' field'")

        if kind == "predictive_y":  # i.e., bivariate t-distributions
            for ibon, bon in enumerate(task_list):
                ...
                plot_confregion_bivariate_t(ax=ax, mu=params["mu"], 
                                ...
                                plot_scatter=False, validate=validate, edgecolor=color, facecolor="None")
        else:  # distributions other than the posterior predictive and prior predictive
            for ibon, bon in enumerate(task_list):
    ...
    @staticmethod
    def place_legend(ax, ncol=1, **kwargs):
        ax.legend(ncol=ncol, prop={'size': 8}, frameon=False, **kwargs)
